...
import { Phone, MapPin, Heart, AlertCircle, Star } from 'lucide-react';
...
export function DivisionHospitalDirectory() {
  ...
  const filterByDivision = (hospital: BaseHospital): boolean => {
    if (selectedDivision === 'All') return true;
    return hospital.region === selectedDivision;
  };

  const filteredHospitals = BASE_HOSPITALS.filter(filterByDivision);
  
  // Group hospitals by division for "All" view
      ...
      <div className="hospitals-list">
        {filteredHospitals.map(hospital => (
          <DivisionHospitalCard 
            key={hospital.id} 
            hospital={hospital} 
            divisionColor={DIVISION_INFO[selectedDivision as keyof typeof DIVISION_INFO]?.color || '#64748b'}
          />
        ))}
      </div>
