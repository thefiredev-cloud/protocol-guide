 ...
 * Formats narrative data for pasting into ImageTrend ePCR tabs
 * 
 * ImageTrend Field Mapping:
 * - Patient Info → eSituation (Chief Complaint, Provider Impression)
 * - Narrative → eNarrative (Free text SOAP format)
 * - Assessment → eExam, eVitals
 * - Medications → eMedications (with NEMSIS codes)
 * - Procedures → eProcedures (with NEMSIS codes)
...
/**
 * Format protocols used for ImageTrend eProtocols section
 * Maps LA County TP codes to NEMSIS protocol format
 */
export function formatProtocols(
  nemsis?: NemsisNarrative,
  carePlan?: CarePlan
): string {
  const lines: string[] = [];
  ...
  // Extract from NEMSIS if available
  if (nemsis?.eProtocols && nemsis.eProtocols.length > 0) {
    nemsis.eProtocols.forEach(p => {
      if (!protocolSet.has(p)) {
        protocolSet.add(p);
        lines.push(`LA County EMS Agency - ${p}`);
      }
    });
  }

  // Extract from care plan if available
  if (carePlan?.protocolReferences && carePlan.protocolReferences.length > 0) {
    carePlan.protocolReferences.forEach(p => {
      if (!protocolSet.has(p)) {
        protocolSet.add(p);
        lines.push(`LA County EMS Agency - ${p}`);
      }
  ...
  if (lines.length === 0) {
    return "No protocols referenced";
  }
...
/**
 * Format transport destination for ImageTrend eDisposition
 */
export function formatTransportInfo(nemsis?: NemsisNarrative): string {
  const lines: string[] = [];

  if (nemsis?.eDisposition) {
    const disp = nemsis.eDisposition;
    if (disp.destinationFacility) {
      lines.push(`Destination: ${disp.destinationFacility}`);
    }
    if (disp.transportMode) {
      lines.push(`Transport Mode: ${disp.transportMode}`);
    }
    if (disp.transportPriority) {
      lines.push(`Priority: ${disp.transportPriority}`);
    }
  }

  return lines.join("\n") || "Transport information not specified";
}
...
export function formatFullExport(
  ...
  const protocols = formatProtocols(nemsis, carePlan);
  ...
  const fullSections = [
    ...
    procedures,
    "",
    "=== LA COUNTY PROTOCOLS USED ===",
    protocols,
    "",
    "--- Generated by County Medic for ImageTrend ePCR ---",
  ];
