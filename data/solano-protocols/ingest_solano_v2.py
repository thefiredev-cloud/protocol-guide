...
Matches existing table schema with columns:
- agency_name, state_code, protocol_number, protocol_title, section
- content, embedding, source_pdf_url, protocol_year
"""
...
def get_voyage_embeddings(texts: list) -> list:
    ...
    # Truncate texts to fit API limits
...
def extract_protocol_number(doc_name: str, short_name: str) -> str:
    ...
    # Look for common patterns: P-001, M-100, etc.
    patterns = [
        r'^([A-Z]-?\d{3,})',  # P-001, M100
        r'^(\d{3,}[-.]?\d*)',  # 101, 101.1
        r'\[([A-Z0-9-]+)\]',  # [P-001]
    ]
    ...
    # Use first 20 chars of name as identifier
...
def delete_existing_chunks():
    ...
    # First count
    count_url = f"{url}?agency_name=eq.{AGENCY_NAME.replace(' ', '%20')}&select=id"
...
def supabase_insert(records: list):
    ...
    if response.status_code not in [200, 201]:
        print(f"Error inserting: {response.status_code} - {response.text[:200]}")
        response.raise_for_status()
    return response
...
def main():
    ...
    for doc in docs_with_text:
        ...
        for chunk_text_content in chunks:
            chunk_record = {
                ...
                'protocol_title': doc_name[:255],  # Truncate if needed
                'section': section,
    ...
    for i in range(0, len(all_chunks), BATCH_SIZE):
        ...
        try:
            ...
            print(f"\r   Progress: {pct}% ({inserted}/{len(all_chunks)}) - {rate:.1f} chunks/sec", end='', flush=True)
