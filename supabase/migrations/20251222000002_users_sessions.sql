...
-- Partial index removed due to NOW() not being immutable
-- Use: SELECT * FROM sessions WHERE user_id = ? AND expires_at > NOW()
CREATE INDEX idx_sessions_user_active ON sessions(user_id, last_activity DESC, expires_at);
