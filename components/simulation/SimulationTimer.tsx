...
// Match the dark theme colors from simulation-section.tsx
const COLORS = {
  primaryRed: "#EF4444",
  textWhite: "#F1F5F9",
  textMuted: "#94A3B8",
...
export const SimulationTimer = memo(function SimulationTimer({
  ...
  const getTimerColor = () => {
    if (!isRunning) return COLORS.textMuted;
    if (isPaused) return COLORS.pausedBlue;
  ...
  const getStatusColor = () => {
    if (!isRunning) return COLORS.textMuted;
    if (isPaused) return COLORS.timerYellow;
    return COLORS.timerGreen;
  };
